...
def install():
    ...
    print(f"\n{Y}(#){C} Radar GRUB theme has been successfully installed !!\n")
...
def uninstall():
    ...
    ask = input(f"{B}(?){C} Remove Radar GRUB Theme (y/n)? {Y}[default = n]{C} : ")
    if ask.lower() != "y":
    ...
    print(f"\n{Y}(#){C} Radar GRUB Theme has been successfully removed !!\n")
    exit()
